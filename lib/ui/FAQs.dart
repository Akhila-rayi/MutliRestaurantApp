...
class FAQsState extends State<FAQs> {
  ...
  Widget getPage(){
    return  BlocListener<AppInfoBloc, AppInfoState>(
        listener: (context, state) {
          ...
          } else if (state is FetchFAQsLoadedState) {
            setState(() {
              showloading = false;
              ...
              if (state.appinfodata != null &&
                ...
                _list.addAll(state.appinfodata);
              } else {
                actualtext = "No FAQ data found";
              }
            });
        ...
        child: Stack(children: <Widget>[
          _list.length > 0
              ? ListView(padding: EdgeInsets.all(8.0), children: [
                  Html(data: _list[0].description.toString(), style: {
                    "body": Style(
  ...
  Future getsaveddata() async{
    ...
    if (isconnected != null && isconnected) {
      _appInfoBloc.add(FetchFAQsEvent());
